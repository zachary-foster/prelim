...
It is estimated that only 0.3% of prokaryotes in soil are culturable [@schleifer2004microbial] and it is likly that obligate symbionts are culturable at even lower rates.
Therefore, studies that rely on culturing microbes from seeds in axenic conditions cannot be used to effectivly test for the presence of such microbes or their effect on plant fitness.
...
Metabarcoding is a relativly recent technique to evaluate the diversity of a particular taxon in a DNA extract from samples such as soils and tissues [@taberlet2012towards].
An varaible region of a gene conserved in a taxon of interest, flanked by conserved regions of DNA, is amplified by PCR using primers that bind to the conserved flanking regions.
The variable amplicons are then sequenced using high-throughput sequening tools, such as the Illumina MiSeq.
...
The relitive number of amplicons sequences that match each organism can then be used as an estimate of realtive abundance of the gene copies in the sample [@nichols2018minimizing], which somewhate correlates to the abundance of organism in the sample.
Although metabcaroding has its own set of biases, espicially when used to estimate the relative abundance of organims, it is an incredably sensitive technqie that cirvumvents the limits of culturability and has already revolutionized the understanding of microbial diversity.
...
1. Dtermine if the differences microbiomes of coocuring plants in these ecosystems correlate with diffrenes in relatedness in thier hosts, as would be consisitent with microbiome sturcutring being a heritable trait.
...
1. Characterize the bacterial, fungal, and oomycete diveristy assocaited with conifer seeds and grass seeds, including unculturable microbes and microbes on seed coat for two LTER site that can inform future research..
